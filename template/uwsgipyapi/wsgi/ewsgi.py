...
class HttpMemTarFile(HttpResponse):
    def __init__(self, tarname="", files=[]):
        ...
        if tarname:
            ...
            headers.append(
                ("Content-Disposition", "attachment; filename* = UTF-8''" + tarname)
            )
...
class WsgiServer(object):
    ...
    def __init__(self):
        ...
        self.rules = [
            (m[3:].replace("__", "/"), m) for m in dir(self) if m.startswith("url__")
        ]
    ...
    def generate_doc(cls):
        ...
        for m in apis:
            ...
            defaults = (
                [] if spec.defaults is None else [(True, i) for i in spec.defaults]
            )
            ...
            kwdefaults = (
                ...
                else [(True, i) for i in spec.kwonlydefaults]
            )
            kwdefaults = [(False, None)] * (
                len(spec.kwonlyargs) - len(kwdefaults)
            ) + kwdefaults
        ...
        if w:
            ...
            if environ["REQUEST_METHOD"] == "POST":
                ...
                ctype, pdict = cgi.parse_header(
                    environ.get("HTTP_CONTENT_TYPE", environ.get("CONTENT_TYPE", ""))
                )
                ...
                elif ctype.startswith("application/json") or ctype.startswith(
                    "text/plain"
                ):
    ...
    def nonblock_parse_multipart(self, wsgi_inpt, pdict, e):
        ...
        with open(tmpfile, "rb") as fp:
            ...
            r = dict(
                [
                    (self.parse_fieldstorage(k, fs[k]))
                    for k in fs.keys()
                    if not k.startswith("_")
                ]
            )
                ...
                if type(p) == type and k in args:
                    try:
                        if p == list:
                            ...
                            args[k] = datetime.datetime.strptime(
                                args[k], "%Y-%m-%d"
                            ).date()
